...
from khrylib.rl.core.policy_cond_gaussian import CondPolicyGaussian
...
parser.add_argument('--change_goal_freq',type=int, default=3)
...
exp_sch = ExpertScheduler(n_experts = env.get_num_expert(), expected_freq=args.change_goal_freq)
...
policy_net = policy_dict[args.policy](MLP(state_dim + (0 if args.policy!= 'cond' else 8), cfg.policy_hsize, cfg.policy_htype), 
...
def main_loop():
    ...
    if args.render:
        ...
        average_length = np.zeros(agent.env.get_num_expert())
        for i_iter in range(args.iter, cfg.max_iter_num):
            ...
            # Adaptive scheduling.s
            exp_sch.record_eps_len(log.avg_episode_len, agent.env.get_expert_id())
            print("Now expert is: ", agent.env.get_expert_id())
            if exp_sch.update(agent.env.get_expert_id(),i_iter):
