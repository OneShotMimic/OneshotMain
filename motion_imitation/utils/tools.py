...
def get_expert(expert_qpos, expert_meta, env):
    ...
    # remove offset 
    expert_qpos[:,:2] += np.random.normal(size=2) * 0.8
...
def get_expert_nimble(expert_qpos, expert_meta, env):
    old_state = env.mj_sim.get_state()
    expert = {'qpos': np.hstack([expert_qpos[:,:7], expert_qpos[:,7:][:,env.mj_nonfoot]]), 'meta': expert_meta}
    feat_keys = {'qvel', 'rlinv', 'rlinv_local', 'rangv', 'rq_rmh',
                 'com', 'head_pos', 'ee_pos', 'ee_wpos', 'bquat', 'bangvel'}
    for key in feat_keys:
    ...
    if expert_meta['cyclic']:
        expert['init_heading'] = get_heading_q(expert_qpos[0, 3:7])
        expert['init_pos'] = expert_qpos[0, :3].copy()
    env.mj_sim.set_state(old_state)
    env.mj_sim.forward()
    return expert