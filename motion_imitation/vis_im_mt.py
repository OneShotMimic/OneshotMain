...
if args.simulator == "mujoco":
    env = HumanoidEnv(cfg) # Humanoid model should based on configuration file.
...
policy_net = policy_dict[args.policy](MLP(state_dim+(0 if args.policy != 'cond' else 8), cfg.policy_hsize, cfg.policy_htype), 
                                      action_dim, log_std=cfg.log_std, fix_std=cfg.fix_std, goal_dim=39, summarize_action=True)
env.switch_expert(args.expert_id)
goal = env.get_goal()
#print("Goal: ", goal)
policy_net.set_goal(goal)
...
class MyVisulizer(Visualizer):
    ...
    def data_generator(self):
        ...
        while True:
            ...
            if args.policy != "mlp":
                sim_mat = policy_net.summary_action()
                print("Action similarity matrix:\n",sim_mat)
                np.save(f"simmat_{args.exp_name}.npy", sim_mat)
