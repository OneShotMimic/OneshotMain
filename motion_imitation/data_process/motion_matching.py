...
parser.add_argument('--amc_id2', type=str, default=None)
...
amc_files = [f'data/cmu_mocap/amc/{args.amc_id}.amc',
             f"data/cmu_mocap/amc/{args.amc_id2}.amc"]
cyclic = False
cycle_offset = 0.0
offset_z = 0.0

# return the final root pose in world coordinate
def get_q_offset(qpos):
    return qpos[:7]

# Assume starting frame of each traj is 0 and heading is 0
# q offset should be end pose
def apply_offset(qpos, q_offset, init_offset):
    qpos[:3] -= init_offset[:3]
    # Apply quaternion rotation
    q = get_heading_q(q_offset[3:7])
    init_q = quaternion_inverse(get_heading_q(init_offset[3:7]))
    qpos[:3] = quat_mul_vec(q, qpos[:3]) + q_offset[:3]
    qpos[3:7] = quaternion_multiply(init_q,qpos[3:7])
    qpos[3:7] = quaternion_multiply(q, qpos[3:7])
    return qpos
...
def convert_amc_file():
    ...
    expert_traj = []
    # The root pose of next file should align with this file
    for i in range(len(amc_files)):
        poses, bone_addr = load_amc_file(amc_files[i], scale)
        if args.fix_feet:
        ...
        init_pose = get_qpos(poses_samp[0])[:7].copy()
        for cur_pose in poses_samp:
            cur_qpos = get_qpos(cur_pose)
            if i != 0:
                cur_qpos = apply_offset(cur_qpos, q_offset, init_pose)
            expert_traj.append(cur_qpos)
        q_offset = get_q_offset(cur_qpos)
...
def visualize():
    ...
    def key_callback(key, action, mods):
        ...
        if key == glfw.KEY_D:
            ...
            if select_end > select_start:
                #expert_traj = expert_traj[select_start: select_end, :]
                first_half = expert_traj[:select_start,:]
                second_half = expert_traj[select_end:,:]
                init_pos = second_half[0][:7].copy()
                q_offset = first_half[-1][:7].copy()
                # for i in range(len(second_half)):
                #     second_half[i] = apply_offset(second_half[i], q_offset, init_pos)
                second_half[:,:3] -= (init_pos[:3] - q_offset[:3])
                expert_traj = np.vstack([first_half, second_half])
                #g_offset += select_start
                select_end = len(expert_traj) - 1
                select_start = 0
                fr = 0
        elif key == glfw.KEY_V:
...
print("Trajectory Length:",select_end)
g_offset = 0
if args.render:
    visualize()
